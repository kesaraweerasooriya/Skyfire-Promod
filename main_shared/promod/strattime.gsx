...
stratTimer() {
    ...
    if (isDefined(game["PROMOD_KNIFEROUND"]) && game["PROMOD_KNIFEROUND"]) matchStartText setText("Waiting For Players For Knife Round");
...
{
	matchWaitText = createServerFontString("objective", 1.5);
    matchWaitText setPoint("CENTER", "CENTER", 0, -60);
    matchWaitText.sort = 1001;
    matchWaitText setText("Waiting for players");
    matchWaitText.foreground = false;
    matchWaitText.hidewheninmenu = false;

	matchWaitTimer = createServerFontString("objective", 1.5);
    matchWaitTimer setPoint("CENTER", "CENTER", 0, -45);
    matchWaitTimer.sort = 1001;
    matchWaitTimer setTimer(maxTime);
    matchWaitTimer.foreground = false;
    matchWaitTimer.hidewheninmenu = false;
	
    wait maxTime;

    level notify("wait_over");

    if (isDefined(matchWaitText)) matchWaitText destroy();
    if (isDefined(matchWaitTimer)) matchWaitTimer destroy();
}*/