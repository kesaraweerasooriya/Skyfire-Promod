...
main() {
    ...
    for (i = 0; i < level.players.size; i++) level.players[i] ShowScoreBoard();
    game["state"] = "postgame";
...
clientHUD() {
    ...
    if (game["promod_first_readyup_done"] && game["promod_in_timeout"] && (!isDefined(game["LAN_MODE"]) || !game["LAN_MODE"])) text = "Remaining";
    else text = "Elapsed";
