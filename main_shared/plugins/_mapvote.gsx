...
{	
	...
	hud[3] setText("The SF Map Voter");
	hud[3].color = (0.402, 0.937 ,0.464);
	...
	hud[4] SetTenthsTimer(10);
	hud[4] thread fadeIn(.3);
	
	//blue top bg
	hud[5] = addTextHud( level, 0, level.windowheight/-2+5, .8, "center", "bottom", "center", "middle", 2.2, 101 );
	hud[5].color = (1, 0.41 ,0.29);
	hud[5] SetShader("white",level.windowwidth+level.borderwidth+level.borderwidth,50);
...
{ 	
	setDvar("timescale",1);
	setDvar( "nextmap","map " + strTok(level.winning,";")[0] );
	exitLevel(false);
}
