...
init() {
	...
	level.players = [];
	level.shoutbars = [];
	level.maxplayers = getDvarInt("sv_maxclients") - getDvarInt("sv_privateclients");
	registerDvars();
...
endGame(winner, endReasonText) {
	...
	if((level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd) {
		...
		if(!hitRoundLimit() || !hitScoreLimit()){
			level thread maps\mp\gametypes\_finalkillcam::startFK( winner );
		} 
...
/*	wait 6;
	...
	thread plugins\_mapvote::init();
	wait 30;*/

	wait 5;
	nextMap = getNextMap();

	if( isDefined(nextMap) && nextMap != "" && isSubStr(nextMap,"mp_") ){
		nextMapName = getMapNameString(nextMap);
	}
	else{
		nextMap = "mp_crossfire";
		nextMapName = "Crossfire";
	}

	setDvar("nextmap", "map " + nextMap);
	exec("say Next Map: " +nextMapName);
	setDvar("svr_mapVoted", 0);
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime) {
	...
	self promod\shoutcast::updatePlayer();
	//level thread codxql\callbacks::CallBack_OnDamage(self, eAttacker, iDamage, sWeapon, sMeansOfDeath);
}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration) {
	if(!isDefined(level.rdyup)) level.rdyup = false;
	self thread maps\mp\gametypes\_finalkillcam::onPlayerKilled(attacker);	
	//if(attacker!=self)level thread plugins\notification::modNotify(attacker, "First Blood");
	//level thread codxql\callbacks::CallBack_OnKill(self, attacker, sWeapon, sMeansOfDeath);
	self endon("spawned");
