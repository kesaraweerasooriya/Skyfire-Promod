...
getSoundPrefixForTeam() {
  ...
  if (self.pers["team"] == "allies") {
    if (game["allies"] == "sas") a = "UK";
    else a = "US";
  } else {
    if (game["axis"] == "russian") a = "RU";
    else a = "AB";
  }
  return a + "_";
}
doQuickMessage(t, i) {
  if (self.sessionstate == "playing" && isdefined(self.pers["team"]) && self.pers["team"] != "spectator" && !isdefined(self.spamdelay)) {
    ...
    if (i >= 0 && i < maxsize) {
      ...
      if (isdefined(level.QuickMessageToAll) && level.QuickMessageToAll) self sayAll(saytext);
      else {
...
quickpromod(response) {
  ...
  case "1":
    ...
    break;
  case "2":
    if (self.sessionstate == "playing" && (!isDefined(self.isPlanting) || !self.isPlanting) && !level.gameEnded && isDefined(self.carryObject)) self.carryObject thread maps\mp\gametypes\_gameobjects::setDropped();
    break;
  case "3":
    self suicide();
    break;
  case "4":
    a = "en";
    if (self promod\client::toggle("PROMOD_RECORD")) a = "dis";
  ...
  case "5":
    ...
    break;
  case "silencer":
    if (self.pers["team"] != "axis" && self.pers["team"] != "allies" || !isDefined(self.pers["class"]) || !getDvarInt("attach_allow_" + self.pers["class"] + "_silencer") || self.pers[self.pers["class"]]["loadout_primary"] == "mp44" || self.pers["class"] == "sniper" || self.pers["class"] == "demolitions") return;
    attach = "none";
    ...
    break;
  case "grenade":
    if (self.pers["team"] != "axis" && self.pers["team"] != "allies" || !isDefined(self.pers["class"])) return;
  ...
  case "sniper":
    ...
    if (!isDefined(self.pers["class"]) || self.pers["class"] != response) self iprintln(chooseClassName(response) + " selected");
  ...
  case "X":
    ...
    break;
  case "shoutcaster":
    if (self.pers["team"] == "spectator") self openMenu(game["menu_shoutcast"]);
    break;
  case "overview":
    if (self.pers["team"] == "spectator") self openmenu(game["menu_shoutcast_map"]);
    break;
  case "controls":
...
quickpromodgfx(response) {
  ...
  case "6":
    ...
    break;
  case "7":
    self setclientdvar("r_fullbright", self promod\client::toggle("PROMOD_FPS"));
    break;
  }
}
chooseClassName(classname) {
  if (!isDefined(classname)) return "";
  switch (classname) {
  case "assault":
    return "Assault";
  case "specops":
